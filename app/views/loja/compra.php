...
<label style='color: green; font-size: 20px'> Onlinne: </label> <label style='color: blue; font-size: 20px'><?php echo($_SESSION['a']); ?>  </label> <?php
	
	$comprador = $_SESSION['a'];  
	$prod = $_POST["nome"];
	$preço = $_POST["preço"];
	$quant = $_POST["quant"];
	$dinheiro = $preço*$quant;
	
	$in = "insert into produtos(comprador, produto, preco, quant, dinheiro ) values ('$comprador', '$prod', '$preço', '$quant', '$dinheiro');";

	$stmt = $conexao->prepare($in);
		

	if ($stmt->execute()) {
			echo "<script> 
	alert('Produto comprado com sucesso');</script>";
	?>
...
<nav >
	...
	//header("Location:loja cliente.html")	
